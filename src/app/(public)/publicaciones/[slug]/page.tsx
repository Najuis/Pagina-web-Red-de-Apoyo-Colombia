...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const post = await getPost(slug, "PUBLICACION");
  if (!post) return {};
  return {
    title: post.title,
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <nav className="mb-6 flex items-center gap-1 text-sm text-muted-foreground" aria-label="Miga de pan">
        ...
        <Link href="/publicaciones" className="transition hover:text-foreground">
          Publicaciones
        </Link>
        <ChevronRight className="size-4" aria-hidden="true" />
      ...
      <div className="mt-10 border-t pt-6 text-center">
        <Link href="/publicaciones" className="text-sm font-medium text-primary hover:underline">
